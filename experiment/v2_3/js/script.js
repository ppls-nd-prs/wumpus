...
function restart(){
    ...
    let env_size = 4;
    let n_wumpusses = Math.round(0.05*(env_size*env_size));
    let n_arrows = 1;
    let n_pits = Math.round(0.05*(env_size*env_size));


    env = new Environment(env_size,env_size,64,64,n_wumpusses,n_pits);

    player = new Player(env, 0, 0, n_arrows);
